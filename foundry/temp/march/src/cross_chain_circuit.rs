...
use crate::proof::{u256_to_bn254, primitive_to_ethnum_u256};
...
impl Define<BN254Config> for CrossChainCircuit {
    fn define(&self, builder: &mut RootBuilder<BN254Config>) {
        ...
        let mut level_index = self.leaf_index; // Use the provided leaf_index
        ...
        for sibling in &self.merkle_proof {
            ...
            let inputs = if level_index % 2 == 0 {
                vec![current_fr, sibling_fr] // Left child
            } else {
                vec![sibling_fr, current_fr] // Right child
            };
            ...
            level_index /= 2; // Move up the tree
        }
