...
fn main() {
    let _previous_proofs = vec![Proof { bytes: vec![0xAA; 32] }];
    ...
    let expected_output = U256::from(990) * scale / U256::from(100); // 9.9 tokens (expected)
    ...
    println!("\n===== Testing GKR-Only Proof (Liquidity) =====");
    ...
    for input_type in 0..3 {
        println!("\n===== Testing Hybrid GKR + Groth16 Proof ({}) =====", type_titles[input_type]);
        let (old_state_root, new_state_root, leaf, merkle_proof) = if input_type == 2 {
            ...
            // Updated to match the computed root from BN254 Poseidon
            let new_state_root = U256::from_dec_str("15476781741752198897071680012448181002077455103560390336381891733571083241623").unwrap();
            let leaf = U256::from(5);
            let merkle_proof = new_tree.get_proof(0);
            (old_state_root, new_state_root, leaf, merkle_proof)
        } else {
            (U256::zero(), U256::zero(), U256::zero(), vec![])
        };
        ...
        let (hybrid_gkr_proof, solidity_proof, public_inputs, verifying_key) = generate_proof(
            ...
            liquidity_added,
            U256::from(5),    // slippage_tolerance (5%)
            expected_new_total,
            user_balance,
            pool_total_liquidity,
            &previous_proofs,
